...
namespace mag
{
    ...
    {
        MAG_ASSERT(false, "[Physics] {0}:{1}: ({2}) {3}", file, line, expression, (message != nullptr ? message : ""));
    ...
    class DebugRenderer : public JPH::DebugRenderer
    {
        public:
            ...
            {
                math::Line line = {};
                line.start = physics::to_mag(inFrom);
                line.end = physics::to_mag(inTo);
                line.color = physics::to_mag(inColor);
