    ...
    class JoltPhysicsWorld : public IPhysicsWorld
    {
        public:
            ...
            RigidBodyHandle add_rigid_body_base(const JPH::ShapeSettings::ShapeResult& shape_result,
                                                const math::vec3& position, const math::quat& rotation, const f32 mass)
            {
                const RigidBodyHandle handle = create_handle();

                // The main way to interact with the bodies in the physics system is through the body interface. There
                // is a locking and a non-locking variant of this. We're going to use the locking version (even though
