...
workspace "magnolia"
    ...
    lib_links = 
    {
        "fmt", "imgui", "imguizmo", "implot", "assimp", "meshoptimizer", "lua",
...
project "lua"
    ...
    if os.host() == "windows" then
        os.execute("echo @TODO WINDOWS LUA")

    -- lua generates the obj files in the same folder as the src, so we copy everything to the build folder
    -- to keep the src folder clean 
    elseif os.host() == "linux" then
        if exists(libdir .. "/liblua.a") then
            os.execute("echo Skipping lua compilation...")
        else
